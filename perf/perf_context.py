...
class PerfNodeEntry[NodeType: HasId, CacheResultType: Any = Any]:
    run: PerfRunContext[NodeType]
    node: NodeType
    parent: Self | None
    ...
    cache_res: CacheResultType | None = field(init=False)
    ...
    def mark_cache(self, cache_res: CacheResultType):
        assert self.cache_res is None
        self.cache_res = cache_res


@dataclass(slots=True)
class PerfRunContext[NodeType: HasId, CacheResultType: Any = Any]:
    ctx: PerfContext[NodeType, CacheResultType]
    roots: list[PerfNodeEntry[NodeType, CacheResultType]] = field(init=False, default_factory=list)
    tip: PerfNodeEntry[NodeType, CacheResultType] | None = field(init=False, default=None)
    ...
    def enter(self, node: NodeType) -> PerfNodeEntry[NodeType, CacheResultType]:
    ...
    def _dump_recurse(self, siblings: list[PerfNodeEntry[NodeType, CacheResultType]], depth: int, table: Table) -> None:
        for pse in siblings:
...
class PerfContext[NodeType: HasId, CacheResultType: Any = Any](contextlib.AbstractContextManager):
    ...
    def exec_node(self, node: NodeType) -> Generator[None | PerfNodeEntry[NodeType, CacheResultType]]:
        if not self.enabled:
            yield None
            return
    ...
    def record_cache(self, node: NodeType, is_hit: bool):
        if not self.enabled:
            return
