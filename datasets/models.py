...
class DatasetSchemaQuerySet(PermissionedQuerySet['DatasetSchema']):
    def for_scope_type(self, scope_model: type[DatasetSchemaScopeType]) -> Self:
    ...
    def for_model(self, scope_model: type[DatasetSchemaScopeType]) -> Self:
        return self.for_scope_type(scope_model)
...
class DatasetQuerySet(PermissionedQuerySet['Dataset']):
    def with_schema_editability(self, shared_scope_type: type[DatasetSchemaScopeType]) -> Self:
        """Load structural editability without per-dataset scope or sibling queries."""
        return self.select_related('schema').annotate(
            _schema_is_shared=models.Exists(
                DatasetSchema.objects.get_queryset().for_scope_type(shared_scope_type).filter(pk=models.OuterRef('schema_id')),
            ),
            _schema_has_other_datasets=models.Exists(
                Dataset.objects.filter(schema_id=models.OuterRef('schema_id')).exclude(pk=models.OuterRef('pk')),
            ),
        )
...
class Dataset(RevisionMixin, UserModifiableModel, UUIDIdentifiedModel, PermissionedModel):
    ...
    # Generated by QS.with_schema_editability
    _schema_is_shared: bool
    _schema_has_other_datasets: bool
    ...
    def __str__(self):
        ...
        label = self.identifier or str(self.uuid)
