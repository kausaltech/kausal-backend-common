"""
This type stub file was generated by pyright.
"""

from graphene.relay import ConnectionField
from graphene.types import Field

class DjangoListField(Field):
    def __init__(self, _type, *args, **kwargs) -> None: ...
    @property
    def type(self):  # -> Any:
        ...
    @property
    def model(self): ...
    def get_manager(self): ...
    @staticmethod
    def list_resolver(django_object_type, resolver, default_manager, root, info, **args):  # noqa: ANN205
        ...
    def wrap_resolve(self, parent_resolver):  # -> partial[QuerySet[Any, Any] | Any]:
        ...

class DjangoConnectionField(ConnectionField):
    def __init__(self, *args, **kwargs) -> None: ...
    @property
    def type(self):  # -> NonNull:
        ...
    @property
    def connection_type(self):  # -> Any:
        ...
    @property
    def node_type(self):  # -> Any:
        ...
    @property
    def model(self):  # -> Any:
        ...
    def get_manager(self):  # -> Any:
        ...
    @classmethod
    def resolve_queryset(cls, connection, queryset, info, args): ...  # noqa: ANN206 type stubs
    @classmethod
    def resolve_connection(cls, connection, args, iterable, max_limit=...):  # -> ConnectionType:  # noqa: ANN206 type stubs
        ...
    @classmethod
    def connection_resolver(  # type: ignore[override]  # noqa: ANN206 type stubs
        cls, resolver, connection, default_manager, queryset_resolver, max_limit, enforce_first_or_last, root, info, **args
    ): ...
    def wrap_resolve(self, parent_resolver):  # -> partial[Promise[Any] | ConnectionType]:
        ...
    def get_queryset_resolver(self):  # -> Callable[..., Any]:
        ...
