...
class BasePageManager(models.Manager[_PageModel], Generic[_PageModel]):
    ...
    # Wagtail builds this manager with `Manager.from_queryset(PageQuerySet)`, so every queryset
    # method is available on it. The ones actually used from the manager are declared here.
    def descendant_of(self, other, inclusive: bool = False) -> PageQuerySet[_PageModel]: ...
    def ancestor_of(self, other, inclusive: bool = False) -> PageQuerySet[_PageModel]: ...
    def child_of(self, other) -> PageQuerySet[_PageModel]: ...
    def live(self) -> PageQuerySet[_PageModel]: ...
    def type(self, *types: type[Page]) -> PageQuerySet[_PageModel]: ...
